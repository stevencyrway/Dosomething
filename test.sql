...
Select campaign_info.campaign_id,
       ...
       action_types,
       scholarship,
       sum(case when is_volunteer_credit = true then reportbacks.hours_spent else null end) as VolunteerCreditTrueVolunteerHours,
       sum(case when is_volunteer_credit = false then reportbacks.hours_spent else null end) as VolunteerCreditFalseVolunteerHours,
       SUM(case
...
AS SELECT
        ...
        (
          ...
          FROM
            public.gambit_messages_inbound i
          LEFT JOIN
            (
              SELECT
              ...
              FROM
                public.bertly_clicks c
              GROUP BY
                c.northstar_id, DATE_TRUNC('month', c.click_time)
...
with sms_funnel as (
    SELECT
      ...
      FROM
        public.gambit_messages_outbound o
      UNION ALL
      SELECT
        i.message_id as event_id,
        i.user_id,
        i.created_at,
        i.macro,
        i.broadcast_id,
      ...
      UNION ALL
      SELECT
        c.click_id as event_id,
        c.northstar_id as user_id,
        c.click_time as created_at,
        NULL AS macro,
        CASE WHEN c.broadcast_id LIKE 'id=%' THEN REGEXP_REPLACE(c.broadcast_id, '^id=', '') ELSE c.broadcast_id END AS broadcast_id,
        null as campaign_id,
        c.interaction_type AS content_type,
        --null as message_text,
        CASE WHEN c.interaction_type IN ('click','uncertain') THEN 'sms_link_click' ELSE 'sms_link_preview' END AS event_type
...
SELECT
    (TO_CHAR(DATE_TRUNC('month', sms_funnel.created_at ), 'YYYY-MM')) AS "sms_funnel.created_at_month",
    COUNT(DISTINCT (CASE WHEN sms_funnel.event_type = 'sms_outbound_message'
        THEN sms_funnel.event_id
        ELSE NULL
        END) ) AS "sms_funnel.count_messages_outbound",
    COUNT(DISTINCT (CASE WHEN sms_funnel.event_type = 'sms_inbound_message'
        THEN sms_funnel.event_id
        ELSE NULL
        END) ) AS "sms_funnel.count_messages_inbound",
    COUNT(DISTINCT (CASE WHEN sms_funnel.event_type = 'sms_link_click'
        THEN sms_funnel.event_id
        ELSE NULL
        END) ) AS "sms_funnel.count_link_clicks"
FROM sms_funnel
WHERE ((sms_funnel.macro ) <> 'subscriptionStatusStop' OR (sms_funnel.macro ) IS NULL) AND ((( sms_funnel.created_at  ) >= ((SELECT (DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL))) AND ( sms_funnel.created_at  ) < ((SELECT ((DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL) + (3 || ' month')::INTERVAL))))) AND ((( sms_funnel.created_at  ) >= ((SELECT (DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL))) AND ( sms_funnel.created_at  ) < ((SELECT ((DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL) + (3 || ' month')::INTERVAL)))))
GROUP BY
    (DATE_TRUNC('month', sms_funnel.created_at ))
ORDER BY
    1 DESC

----SMS Messages Sent
SELECT
    (TO_CHAR(DATE_TRUNC('month', "created_at"), 'YYYY-MM')) AS "sms_outbound.created_at_month",
    COUNT(DISTINCT sms_outbound.message_id ) AS "sms_outbound.count"
FROM
    "public"."gambit_messages_outbound" AS "sms_outbound"
WHERE LENGTH("broadcast_id") <> 0 AND ((( "created_at" ) >= ((SELECT (DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL))) AND ( "created_at" ) < ((SELECT ((DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL) + (3 || ' month')::INTERVAL))))) AND "broadcast_id" IS NOT NULL
GROUP BY
...
---Bertly Clicks
SELECT
    (TO_CHAR(DATE_TRUNC('month', "click_time"), 'YYYY-MM')) AS "bertly_clicks.click_month",
    COUNT(DISTINCT bertly_clicks.click_id ) AS "bertly_clicks.click_count"
FROM
    "public"."bertly_clicks" AS "bertly_clicks"
WHERE LENGTH("broadcast_id") <> 0 AND ((( "click_time" ) >= ((SELECT (DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL))) AND ( "click_time" ) < ((SELECT ((DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL) + (3 || ' month')::INTERVAL))))) AND ((( "click_time" ) >= ((SELECT (DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL))) AND ( "click_time" ) < ((SELECT ((DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL) + (3 || ' month')::INTERVAL))))) AND "broadcast_id" IS NOT NULL AND "broadcast_id" NOT LIKE '%&source=sms%'
GROUP BY
    (DATE_TRUNC('month', "click_time"))
ORDER BY
    2 DESC
...
SELECT
    cio_campaign_name as "Email Campaign Name",
    (DATE_TRUNC('week', "timestamp")) AS "email_funnel.created_at_week",
    COUNT(DISTINCT (CASE WHEN email_funnel.event_type = 'email_sent'
        THEN email_funnel.email_id
        ELSE NULL
        END) ) AS "Emails sent",
    COUNT(DISTINCT (CASE WHEN email_funnel.event_type = 'email_opened'
        THEN email_funnel.email_id
        ELSE NULL
        END) ) AS "Email Opens",
    COUNT(DISTINCT (CASE WHEN  "event_type"  = 'email_clicked'
        ...
        ELSE NULL
        END) ) AS "Email Clicks",
    COUNT(DISTINCT (CASE WHEN email_funnel.event_type = 'email_unsubscribed'
        THEN email_funnel.email_id
        ELSE NULL
        END) ) AS "Email Unsubscribes",
    COUNT(DISTINCT (CASE WHEN email_funnel.event_type = 'email_bounced'
        THEN email_funnel.email_id
        ELSE NULL
        END) ) AS "Emails Bounced"
FROM
    "public"."cio_email_events" AS "email_funnel"
WHERE ((( "timestamp" ) >= ((SELECT (DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL))) AND ( "timestamp" ) < ((SELECT ((DATE_TRUNC('month', DATE_TRUNC('day', CURRENT_TIMESTAMP AT TIME ZONE 'UTC')) + (-2 || ' month')::INTERVAL) + (3 || ' month')::INTERVAL)))))
GROUP BY
    (DATE_TRUNC('week', "timestamp")), cio_campaign_name
...
AS SELECT
    ...
    FROM
      ...
      FROM
        public.cio_email_events
      GROUP BY
        customer_id,
        DATE_TRUNC('month', TIMESTAMP)
      ) efacts

Select * from public.cio_email_events
join gambit_messages_inbound on user_id = customer_id
...
Select * from user_newsletter_subscriptions
select * from email_event
select * from cio_email_events
where cio_campaign_id is not null
